...
class TGNMemory(torch.nn.Module):
    ...
    def _reset_message_store(self):
        # NOTE (CADETS_E5 patch): the message store is kept on CPU. It holds one
        # (src, dst, t, msg) tuple per touched node, so on a 5.2M-node graph the
        # per-node GPU slices (~4 tensors each) exhaust the 10GB GPU through
        # allocator overhead alone during long inference passes. Values are moved
        # back to the memory device inside `_compute_msg`, so semantics are unchanged.
        i = torch.empty((0,), dtype=torch.long, device="cpu")
        msg = torch.empty((0, self.raw_msg_dim), device="cpu")
    ...
    def _update_msg_store(
        ...
        for i, idx in zip(n_id.tolist(), perm.split(count.tolist())):
            # NOTE (CADETS_E5 patch): store CPU slices — see `_reset_message_store`.
            msg_store[i] = (src[idx].cpu(), dst[idx].cpu(), t[idx].cpu(), raw_msg[idx].cpu())

    def _compute_msg(self, n_id: Tensor, msg_store: TGNMessageStoreType, msg_module: Callable):
        data = [msg_store[i] for i in n_id.tolist()]
        src, dst, t, raw_msg = list(zip(*data))
        # NOTE (CADETS_E5 patch): the store lives on CPU, so move back to the
        # memory device before any GPU op (concat/scatter/GRU below).
    ...
    def train(self, mode: bool = True):
        ...
        if self.training and not mode:
            ...
            # self._update_memory(...) REMOVED: full-node GRU update OOMs on CADETS_E5 (per upstream comment)
            self._reset_message_store()
        super().train(mode)
