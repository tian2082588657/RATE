...
def main():
    ...
    ap.add_argument("--extra-enc", default="",
                    help="额外按归约图跑一组的编码列表（逗号分隔），"
                         "配置名写作 TeRed+RATE*")
    ap.add_argument("--extra-id-enc", default="",
                    help="额外按【未归约图】跑一组的编码列表（逗号分隔），"
                         "配置名写作 identity+<enc>；用于等宽对照")
    ap.add_argument("--base-configs", type=int, default=1,
                    help="0 = 只跑 --extra-enc 指定的配置")
    a = ap.parse_args()
    ...
    for spec in specs:
        ...
        if not a.base_configs:
            configs = []
        for _e in [x.strip() for x in a.extra_enc.split(",") if x.strip()]:
            configs.append((f"TeRed+RATE*", [r.Gp for r in res_train_te],
                            res_te, _e))
        for _e in [x.strip() for x in a.extra_id_enc.split(",") if x.strip()]:
            configs.append((f"identity+{_e}", [r.Gp for r in res_train_id],
                            res_id, _e))
        for cname, gpt, res_test, enc in configs:
            ...
            seen = set()
            for c, _ in clusters:
                seen.update(np.nonzero(y[c])[0].tolist())
